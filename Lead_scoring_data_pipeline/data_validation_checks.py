...
def raw_data_schema_check(data_directory, lead_scoring_file, raw_data_schema):
    '''
    ...
    print('columns from the source table',source_cols)
    print('columns from the raw_data_schema',raw_data_schema)
...
def model_input_schema_check(db_path, db_file_name, model_input_schema):
    ...
    print('columns of interactions_mapped' , df.columns)
    
    #df.drop(columns=['index'], inplace=True, axis=1, errors='ignore')
    #print('columns of interactions_mapped After droping sql index' , df.columns)
    
    source_columns = df.columns.to_list()
    print("source_columns", source_columns)
    
    print("model_input_schema", model_input_schema)
